...
$ApkPath = Join-Path $MobileRoot "build-artifact\PII-Pilot-V2.apk"
...
function Stop-Run {
    ...
    Write-Host ("RUN {0}: {1}" -f $Kind, $Message)
    exit 1
}

Write-Host "Android standalone install and launch"
Write-Host ""

if (-not (Test-Path -LiteralPath $ApkPath -PathType Leaf)) {
    Stop-Run "BLOCKED" "PII-Pilot-V2.apk is missing. Run android-dev.ps1 build first."
}
if (-not (Test-Path -LiteralPath $AppJsonPath -PathType Leaf)) {
    Stop-Run "BLOCKED" "mobile app.json is missing."
}
...
try {
    ...
    Stop-Run "BLOCKED" "mobile app.json could not be parsed."
}
if (-not $packageName) {
    Stop-Run "BLOCKED" "expo.android.package is missing."
}
...
if ($blockedLines.Count -gt 0) {
    Stop-Run "BLOCKED" "$($blockedLines.Count) device(s) are offline or unauthorized; identifiers hidden."
}
if ($readyLines.Count -ne 1) {
    Stop-Run "BLOCKED" "Expected exactly one ready device; found $($readyLines.Count); identifiers hidden."
}
$serial = ($readyLines[0] -split "\s+", 2)[0]

Write-Host "[1/3] Installing release APK..."
$install = Invoke-NativeCommand $adbPath @("-s", $serial, "install", "-r", $ApkPath)
if ($install.ExitCode -ne 0 -or $install.Output -notmatch '(?m)^\s*Success\s*$') {
    Stop-Run "FAILED" "APK installation failed; device identifier and raw adb output are hidden."
}
...
if ($forceStop.ExitCode -ne 0) {
    Stop-Run "FAILED" "Could not stop the previous application process."
}
...
if ($launch.ExitCode -ne 0 -or $launch.Output -notmatch 'Events injected:\s*1') {
    Stop-Run "FAILED" "Launcher activity did not start."
}
