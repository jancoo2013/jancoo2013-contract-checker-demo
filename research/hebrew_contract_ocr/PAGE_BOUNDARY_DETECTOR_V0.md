...
- `page_corners.json` that contains accepted corners or an explicit `null` fallback decision
  for every source and can be passed directly to `page_normalizer.py`;
...
Horizontal page edges are limited to modest perspective in v0. A more severely rotated or oblique capture is not rectified speculatively; it follows the full-frame fallback, while a later capture-quality gate may still request recapture if the text is unusable.

No OCR engine, neural model, network call, or cloud service participates in detection.

## 3. Conservative fallback contract

The detector must not invent a crop when no plausible outer quadrilateral exists, an edge is weak, the geometry is implausible, a proposed top edge crosses printed content, or overall confidence is below the gate. Such a page is recorded as rejected for boundary detection, but remains usable as a full-frame OCR input.

Every source is written to `page_corners.json`. Accepted pages have ordered corners; rejected pages have `null`, which instructs the normalizer to preserve the full frame. A missing source key remains an invalid handoff. Rejected pages also remain in the manifest and overlay set so the decision is inspectable.
...
For an accepted quadrilateral, everything outside it is discarded and cannot reach line segmentation or OCR. For an explicit `null` decision, the full available frame is normalized without perspective crop; later text segmentation is responsible for locating text independently of paper edges.
