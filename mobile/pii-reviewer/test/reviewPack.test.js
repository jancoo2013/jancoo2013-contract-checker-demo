...
async function fixture() {
  ...
  const snapshot=async(_bytes,name)=>`memory://${name}`;
  return { files, read, predictionSha, imageSize, snapshot };
}
...
test('valid pack binds manifests, hashes, lines and snapshots',async()=>{ const {read,predictionSha,imageSize,snapshot}=await fixture(); const pack=await loadReviewPack(read,sha,imageSize,snapshot); assert.equal(pack.packKey,predictionSha); assert.deepEqual(pack.pages[0].candidateMasks[0].box,[1,2,10,8]); assert.equal(pack.pages[0].reviewRegions[0].id,'P0001-L0001'); assert.match(pack.pages[0].sourceUri,/memory:/); });
