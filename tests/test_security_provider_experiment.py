...
class SecurityProviderExperimentTests(unittest.TestCase):
    def test_prompt_hides_oracle_answers(self):
        case = {
            "lifecycle": "EXECUTED",
            "clauses": [{"ref": "c1", "text_he": "טקסט"}],
            "assertions": [{"question_id": "security.instrument_amounts",
                            "field": "instrument_amounts", "expected": 10000,
                            "refs": ["c1"], "mechanism_id": "security_1"}],
    ...
    def test_exact_scoring(self):
        ...
        actual = {"assertions": [{
            ...
            "value": 10000, "state": dict(experiment.DEFAULT_STATE), "refs": ["c1"],
        }], "resolution": None}
    ...
    def test_handwriting_guess_is_hard_failure(self):
        case = {
            "clauses": [{"ref": "c1", "text_he": "טקסט"}],
            "redactions": [{"ref": "r1", "kind": "HANDWRITING"}],
            "assertions": [{"question_id": "security.instrument_amounts",
                            "field": "instrument_amounts", "expected": None,
                            "refs": ["c1", "r1"], "mechanism_id": "security_1",
                            "state": {"value": "UNKNOWN",
        ...
        self.assertIn("guessed_unavailable_value",
                      experiment.score_case(case, actual)["hard_failures"])
