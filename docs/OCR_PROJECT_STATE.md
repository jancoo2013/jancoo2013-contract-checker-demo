...
Последнее обновление: 2026-07-20, после закрытия Gold/Silver training leakage.
...
Разрешённые владельцем corrective-исключения от 2026-07-20 ограничены передачей `rejected boundary → full frame`, исправлением no-upscale и закрытием Gold/Silver leakage. Они не добавляют новый OCR-компонент; после их слияния единственным следующим шагом остаётся Gold Set v0.
